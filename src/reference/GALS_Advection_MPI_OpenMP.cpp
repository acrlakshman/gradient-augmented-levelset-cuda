...
int main(int argc, char **argv){
    ...
    remove("phi.txt");         remove("psix.txt");         remove("psiy.txt");     remove("psixy.txt");  
    remove("Velocity_x.txt");  remove("Velocity_y.txt");   remove("details.txt");
	...
	A1 = alloc_2d_int(nnx,ny);      B1 = alloc_2d_int(nnx,ny);
	A3 = alloc_2d_int(nnx,ny);	B3 = alloc_2d_int(nnx,ny);   	C3 = alloc_2d_int(nnx,ny);
   ...
   if(ctx.rank() == 0){
	...
	// fct-1: determine the advection point
	...
	// populate the recevied data to the current data array
	# pragma omp parallel
	{
        # pragma omp for 
	...
	// fct-2: find the location of the advection point 
	...
	// fct-3: calculate the level-set data, here I do NOT use MPI
	...
	// end of fct-3
        ...
        // fct-4: Update the mixed derivatives now for the remaining grid points
	...
	// populate the data, here I transpose the matrix because for c++, matrix is read in row-wise order 
	# pragma omp parallel
	{
        # pragma omp for 
        for(unsigned int i = 0; i < ny; i++){  // loop for y - rows in the array
